...
class Users():
    ...
    def get_req(self):
        ...
        result = db.get_req_db()
        return result

    def del_user(self,id):

        result = db.del_user_db(id)
        return result


    def create_req(self, user_model, method,headers,ip,endpoint,req_url):

        result = db.create_req_db(user_model, method,headers,ip,endpoint,req_url)
        return result

    def get_regs(self):

        result = db.get_regs_db()
        return result

    def reg_user(self, firstName,lastName,password,email,req_url):

        result = db.reg_user_db(firstName,lastName,password,email,req_url)
        return result
