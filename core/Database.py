...
class Database():
    ...
    def __init__(self):
        ...
        mycursor.execute("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_NAME='Socket' and TABLE_SCHEMA='scim'")
        myresult = mycursor.fetchall()
        if not myresult:
            mycursor.execute("CREATE TABLE IF NOT EXISTS `Socket` ( `id` VARCHAR(255) COLLATE utf8_general_ci NOT NULL , `req` json NOT NULL, `method` VARCHAR(255) COLLATE utf8_general_ci NOT NULL , `headers` MEDIUMTEXT COLLATE utf8_general_ci NOT NULL, `ip` VARCHAR(255) COLLATE utf8_general_ci NOT NULL, `endpoint` VARCHAR(255) COLLATE utf8_general_ci NOT NULL, PRIMARY KEY (`id`)) ENGINE = MyISAM;")

        mycursor.execute("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_NAME='Registration' and TABLE_SCHEMA='scim'")
        myresult = mycursor.fetchall()
        if not myresult:
            mycursor.execute("CREATE TABLE IF NOT EXISTS `Registration` ( `id` VARCHAR(255) COLLATE utf8_general_ci NOT NULL , `firstName` VARCHAR(255) COLLATE utf8_general_ci NOT NULL, `lastName` VARCHAR(255) COLLATE utf8_general_ci NOT NULL , `password` MEDIUMTEXT COLLATE utf8_general_ci NOT NULL, `email` VARCHAR(255) COLLATE utf8_general_ci NOT NULL, PRIMARY KEY (`id`)) ENGINE = MyISAM;")
    ...
    def get_req_db(self):
        try:
            mycursor.execute("SELECT * FROM `Socket`")
        except mysql.connector.Error as err:
            status_code = 400
            error_message = "{}".format(err)
            scim_error = scim_core.CreateSCIMEerror(error_message, status_code)
    ...
    def create_req_db(self, user_model, method,headers,ip,endpoint,req_url):

        uid = str(uuid.uuid4())
        req_url = req_url+'/'+uid
        sql = """INSERT INTO `Socket` (id, req,method,headers,ip,endpoint) VALUES (%s, %s,%s, %s,%s, %s)"""
        val = (uid, user_model,method,headers,ip,endpoint)
        ...
        try:
            mycursor.execute("SELECT * FROM `Socket` WHERE id='{}'".format(uid))
        except mysql.connector.Error as err:
            status_code = 400
            error_message = "{}".format(err)
            scim_error = scim_core.CreateSCIMEerror(error_message, status_code)
    ...
    def reg_user_db(self, firstName,lastName,password,email,req_url):

        uid = str(uuid.uuid4())
        req_url = req_url+'/'+uid
        sql = """INSERT INTO `Registration` (id, firstName,lastName,password,email) VALUES (%s, %s,%s, %s,%s)"""
        val = (uid,firstName,lastName,password,email)
        ...
        #scim_core.CreateSCIMUser(uid, True, user_model['userName'], user_model['givenName'], user_model['familyName'], user_model['email'], req_url, user_model['middleName'], user_model['groups'])
        try:
            mycursor.execute("SELECT * FROM `Registration` WHERE id='{}'".format(uid))
        except mysql.connector.Error as err:
            status_code = 400
            error_message = "{}".format(err)
            scim_error = scim_core.CreateSCIMEerror(error_message, status_code)
    ...
    def del_user_db(self, id):
        try:
            mycursor.execute("DELETE FROM `Registration` WHERE id = '{}'".format(id))
